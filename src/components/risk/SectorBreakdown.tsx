...
import { formatPKR, formatPercent, plColor, formatPKRCompact } from '@/lib/formatters';
...
export default function SectorBreakdown({ data }: Readonly<{ data: SectorAllocation[] }>) {
  const pieData = data.map((s) => ({ name: getSectorDisplay(s.sector).name, value: s.current_value }));
          ...
          <tbody>
            {data.map((s, i) => (
              <tr key={s.sector} style={{ background: i % 2 === 0 ? 'transparent' : 'var(--bg-secondary)', borderBottom: '1px solid var(--border-light)' }}>
                <td className="px-4 py-3 font-medium" style={{ color: 'var(--text-primary)' }}>{getSectorDisplay(s.sector).name}</td>
